...
class BabyProfileServiceTest {
    ...
    void upsert_returnsProfile_onSuccess() {
        ...
        when(jdbc.queryForMap(anyString(), eq(USER_ID), eq("Lily"), eq("2025-06-01"), eq("Jane"), eq("555-0100"), isNull(), isNull(), eq("baby")))
            .thenReturn(row);
    ...
    void upsert_passesNullBirthdate_whenBlank() {
        ...
        row.put("parent_name", "Jane");
        row.put("phone",       null);
        when(jdbc.queryForMap(anyString(), eq(USER_ID), eq("Lily"), isNull(), eq("Jane"), isNull(), isNull(), isNull(), eq("baby")))
            .thenReturn(row);

        var req = new BabyProfileRequest("Lily", "", "Jane", null, null, null, "baby");
        BabyProfileResponse result = babyProfileService.upsert(USER_ID, req);
